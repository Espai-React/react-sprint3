...
function generateCart() {
  ...
  for (i = 0; i < cartList.length; i++) {
    ...
    const cartItemIndex = cart.findIndex(
      (element) => element.id === cartListItem.id
    );
...
function applyPromotionsCart() {
  ...
  for (i = 0; i < cart.length; i++) {
    ...
    if (
      (cartItem.id == 1 || cartItem.id == 3) &&
      cartItem.quantity >= cartItem.offer.number
    ) {
      cartItem.subTotalWithDiscount = Number(
        (
          cartItem.subTotal -
          (cartItem.subTotal * cartItem.offer.percent) / 100
        ).toFixed(2)
      );
    }
  }
...
function printCart() {
  ...
  for (i = 0; i < cart.length; i++) {
    ...
    list +=
      "<td class='text-center'><button type='button' class='btn btn-outline-dark btn-sm rounded-pill px-2 py-0' onclick='oneMoreToCart(" +
      cartItem.id +
      ")'>+</button></td>";
    list +=
      "<td class='text-center'><button type='button' class='btn btn-outline-dark btn-sm rounded-pill px-2 py-0' onclick='removeFromCart(" +
      cartItem.id +
      ")'>-</button></td>";
    list += '</tr>';
  }
...
function addToCart(id) {
  ...
  const cartItemIndex = cart.findIndex(
    (element) => element.id === productChosen.id
  );
